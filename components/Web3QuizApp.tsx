        ...
        <div className="border-b-6 border-black bg-white sticky top-0 z-10">
          <div className="max-w-4xl mx-auto px-4 py-4">
            <div className="flex items-center justify-between gap-4 flex-wrap">
              ...
              <div className={`flex items-center gap-2 px-4 py-2 border-4 border-black ${ timeLeft <= 60 ? 'bg-red-400' : 'bg-green-400'}`}>
        ...
        <div className="max-w-4xl mx-auto px-4 py-8">
          <div className="bg-white border-6 border-black p-6 md:p-8 shadow-[12px_12px_0px_0px_rgba(0,0,0,1)] mb-6">
            <div className="flex items-start justify-between gap-4 mb-6">
              ...
              <span className={`text-xs md:text-sm px-3 py-1 border-3 border-black whitespace-nowrap font-black uppercase ${ question.difficulty === 'Easy' ? 'bg-green-400' : question.difficulty === 'Medium' ? 'bg-yellow-400' : 'bg-red-400'}`}>
                {question.difficulty}
              </span>
            </div>
            ...
            <div className="space-y-4">
              {question.options.map((option, index) => (
                <button
                  ...
                  className={`w-full text-left p-4 md:p-5 border-4 border-black transition-all ${ answers[currentQuestion] === index ? 'bg-gradient-to-r from-yellow-300 to-pink-300 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] translate-x-[-2px] translate-y-[-2px]' : 'bg-white shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] hover:shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px]'}`}
                >
                  <div className="flex items-center gap-3">
                    <span className={`flex items-center justify-center w-10 h-10 border-4 border-black text-base md:text-lg font-black ${ answers[currentQuestion] === index ? 'bg-black text-white' : 'bg-white text-black'}`}>
              ...
              {!isWalletConnected ? (
                ...
                <div className="text-center p-6 bg-green-200 border-4 border-black">
                  ...
                  <button
                    ...
                    className={`w-full bg-gradient-to-r from-green-500 to-emerald-600 border-4 border-black px-8 py-4 shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] hover:shadow-[8px_8px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[-2px] hover:translate-y-[-2px] transition-all duration-200 font-black text-lg text-white uppercase ${ isPaymentLoading ? 'opacity-70' : ''}`}
                  >
                    {isPaymentLoading ? (
              ...
              <div className="bg-white border-6 border-black p-6 shadow-[8px_8px_0px_0px_rgba(0,0,0,1)]">
                ...
                <div className="space-y-3">
                  {allTimeTop.map((entry, index) => (
                    <div
                      key={entry.id}
                      className={`flex items-center gap-4 p-4 border-4 border-black ${ index === 0 ? 'bg-gradient-to-r from-yellow-300 to-orange-300' : index === 1 ? 'bg-gradient-to-r from-gray-300 to-gray-400' : index === 2 ? 'bg-gradient-to-r from-orange-300 to-amber-400' : 'bg-gray-100'}`}
                    >
                      <div className={`flex items-center justify-center w-12 h-12 border-4 border-black font-black text-xl ${ index < 3 ? 'bg-black text-white' : 'bg-white text-black'}`}>
                        {index + 1}
                      </div>
